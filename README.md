...
EPM (issue #27) landed the same day, scorporated from an ECS design
discussion rather than folded into it — deliberately out of the Sprint 5
plan above, same principle as Sprint 6 below: added without reordering
what was already planned. Scope: a hotness checkpoint for `EAT`
(`export_snapshot()`/`load_snapshot()`), so `_seed_eat_entries()` doesn't
always cold-start `access_count=0` on every process restart. See the
"Known limitations / open issues" table below (#27) for status — unit-
tested, not yet exercised on real hardware.
...
| [#27](https://github.com/danielesalpietro/vMemoryFabric/issues/27) | EPM — `EAT.shutdown()` clears all hotness on every process restart, `_seed_eat_entries()` always seeds `access_count=0` | Primitive implemented and unit-tested (154 passed): `EAT.export_snapshot()`/`load_snapshot(decay=0.5)`, `GCSGWorker.configure_eat_snapshot()`/`finalize_epm_run()`, default-on with `--no-epm` to disable, run history bounded at 256, snapshot/history filenames scoped to model identity (`epm.snapshot_path_for_model()`). Synthetic benchmark (`benchmarks/bench_epm.py`, no GPU needed) shows the mechanism working as designed (0%→100% cold-start match with a stationary prior; `decay=0.5` reconverges within 2 runs of a hot-expert shift, `decay=1.0` never does). **Not yet exercised on real hardware** — kept open on purpose, same standing as #2/#10/#16, until a real `--wire-tier-manager` rerun confirms `continued_from_previous=True` on actual routing traffic, not simulated. Full trail: `osx-poc/LOGBOOK.md`, 2026-08-13 "EPM (issue #27)" entry |
