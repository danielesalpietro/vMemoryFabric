...
│
├── logs/                        # run logs/artifacts, one subfolder per sprint
...
| — | CUDA-only, no multi-backend support (ROCm/XPU/HPU/Neuron/TPU) | Not yet a GitHub issue — under evaluation for a future evolution. Feasibility study: `osx-poc/reports/multibackend_portability_feasibility.md` (hard blockers: Marlin/AWQ kernels, NVML; soft coupling: `GPUTransfer` device dispatch, portable behind a `vllm.platforms`-style abstraction) |
